...
Cursor:
- Context file: .cursorrules in project root — the user will paste the full PRD (with preamble) here so the AI always has context
...
Lovable:
- Initial prompt: the user will paste the full PRD (with preamble) as the first message — Lovable handles long context well
...
Bolt.new:
- Initial prompt: the user will paste the full PRD (with preamble) as the first message
...
Claude Code:
- CLAUDE.md: create this file in project root with the full PRD (with preamble) — this is the AI's persistent memory
...
Codex (OpenAI):
- Task description: the user will paste the full PRD (with preamble) as the task, let the agent plan the implementation
...
Google AI Studio:
- System instructions: the user will paste the full PRD (with preamble) into the System Instructions field
...
- Prompt structure: the user will paste the full PRD (with preamble) — include design details, colour palette, and component behaviour
...
Replit Agent:
- Project description: the user will paste the full PRD (with preamble) as the initial agent description
...
export const appbuildguide = onRequest({ secrets: [openRouterApiKey] }, async (req, res) => {
  ...
  try {
    ...
    const platformPreamble = typeof data.platform_preamble === "string" ? data.platform_preamble : "";
...
// ═══════════════════════════════════════════════════════════════
// PERPLEXITY GUIDE — Generate a structured Perplexity learning guide
// ═══════════════════════════════════════════════════════════════
...
2. "spaceConfig" — configuration for setting up a Perplexity Space:
   - "spaceName": A suggested name for the Space (e.g. "AI Agent Building — Level 2")
   - "customInstructions": Tailored instructions the learner should paste into the Space's custom instructions field. These should reference their level, topic, and preferred explanation style.
...
export const generateperplexityguide = onRequest(
  { secrets: [openRouterApiKey] },
  async (req, res) => {
    if (req.method !== "POST") { res.status(405).json({ error: "Method not allowed" }); return; }
    const { apiKey, model } = getEnv();
...
REMINDER: Do NOT use "Level 1", "Level 2", etc. anywhere in your output. Describe the learner's stage using natural language.`;
      ...
      const result = await callOpenRouter({
        apiKey,
        model,
        systemPrompt: PERPLEXITY_GUIDE_SYSTEM,
        userMessage,
        label: "generate-perplexity-guide",
        temperature: 0.7,
        jsonMode: true,
      });
      ...
      console.error("generate-perplexity-guide error:", err);
      res.status(500).json({ error: "Internal server error", retryable: true });
    }
  }
);
