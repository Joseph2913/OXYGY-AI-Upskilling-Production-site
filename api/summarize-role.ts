  ...
  try {
    ...
    const geminiResponse = await fetch(geminiUrl, {
      ...
      body: JSON.stringify({
        system_instruction: {
          parts: [{
            text: `You are a concise profile summarizer. Given a user's role description, produce a short professional title (max 8 words). Return ONLY the title text, nothing else. Examples:
